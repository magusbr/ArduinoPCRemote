...
void loop() {
  ...
  if (irrecv.decode(&results)) {
    ...
    Serial.println(key_pressed);
	  ...
	  if (key_pressed == SONY_IR_PLAY) {
      Consumer.write(MEDIA_PLAY_PAUSE);
	    ok_pause = 1;
    }
    else if (key_pressed == SONY_IR_PREV) {
      Consumer.write(MEDIA_VOLUME_UP);
	    ok = 1;
    }
    else if (key_pressed == SONY_IR_FAST_PREV) {
      Consumer.write(MEDIA_VOLUME_DOWN);
	    ok = 1;
    }
    else if (key_pressed == SONY_IR_PAUSE) {
      Consumer.write(MEDIA_RECORD);
      ok_pause = 1;
    }
    else if (key_pressed == SONY_IR_NEXT) {
      Keyboard.write(KEY_LEFT_WINDOWS);
      ok = 1;
    }
    else if (key_pressed == SONY_IR_FAST_NEXT) {
    ...
    else if (key_pressed == SONY_IR_UP) {
      Keyboard.write(KEY_UP_ARROW);
      ok = 1;
    }
    else if (key_pressed == SONY_IR_DOWN) {
      Keyboard.write(KEY_DOWN_ARROW);
      ok = 1;
    }
    else if (key_pressed == SONY_IR_LEFT) {
      Keyboard.write(KEY_LEFT_ARROW);
      ok = 1;
    }
    else if (key_pressed == SONY_IR_RIGHT) {
      Keyboard.write(KEY_RIGHT_ARROW);
      ok = 1;
    }
    else if (key_pressed == SONY_IR_OK) {
